...
import com.pwc.us.rgi.parsergen.ruledef.TEmpty;

public class TestObjectSupply implements ObjectSupply<Token> {
	@Override
	public TTString newString(TextPiece piece) {
		return new TTString(piece);
	}
	
	@Override
	public TTSequence newSequence(SequenceOfTokens<Token> tokens) {
		return new TTSequence(tokens);
	}
	
	@Override
	public TTList newList(ListOfTokens<Token> tokens) {
		return new TTList(tokens);
	}
