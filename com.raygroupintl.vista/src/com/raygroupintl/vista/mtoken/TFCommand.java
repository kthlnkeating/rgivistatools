...
public class TFCommand extends TFSeq {
	...
	private static class TKCommandSpec extends TCommandSpec {
		...
		@Override
		protected ITokenFactory buildArgumentFactory(final MVersion version) {
			return TFCommaDelimitedList.getInstance(TFKillArgument.getInstance(version)); 	
		}
	...
	private static class TLCommandSpec extends TCommandSpec {
		...
		@Override
		protected ITokenFactory buildArgumentFactory(final MVersion version) {
			ITokenFactory tfNRef = ChoiceSupply.get(TFLvn.getInstance(version), "^@", MTFSupply.getInstance(version).getTFGvn(), MTFSupply.getInstance(version).getTFIndirection());		
